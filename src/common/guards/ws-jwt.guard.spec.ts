...
function makeSocket(token?: string, authHeader?: string): Socket {
  return {
    handshake: {
      auth: token ? { token } : {},
...
describe('WsJwtGuard', () => {
  ...
  it('throws WsException when JWT verification fails', () => {
    ...
    expect(() => guard.validateClient(makeSocket('bad-token'))).toThrow(
      WsException,
    );
  });
  ...
  it('accepts a valid token from the Authorization header', () => {
    ...
    guard.validateClient(client);
    expect((client.data as Record<string, unknown>)['userId']).toBe('user-456');
  });
